...
const PieChart = ({ year, month, range }) => {
    ...
    useEffect(() => {
        // 차트 인스턴스 제거
        const chartInstance = pieChartRef.current && Chart.getChart(pieChartRef.current);
        if (chartInstance) {
        ...
        // 데이터 가져오기
        axios({
            url: `http://localhost:8089/qrancae/defectChart`,
            method: 'get',
    ...
    useEffect(() => {
        ...
        // 새로운 차트 데이터와 설정
        const labels = defectData.length > 0 ? defectData.map(item => Object.keys(item)[0]) : ['데이터 없음'];
        const data = defectData.length > 0 ? defectData.map(item => {
        ...
        const chartData = {
            ...
            datasets: [{
                label: 'Pie Chart',
                backgroundColor: defectData.length > 0 ? ['#FF6384', '#36A2EB', '#FFCE56', '#FF9F40'] : ['#FF6384'],
                data: data,
            }],
        };
                        ...
                        defectData.map((item, index) => {
                            ...
                            return (
                                <tr key={label}>
                                    <td>{index + 1}</td>
                                    <td>{label}</td>
                                    <td>{formatPercentage(value * 100)}%</td> {/* 불량률을 백분율로 표시 */}
                                </tr>
                            );
                        })
                    )}
