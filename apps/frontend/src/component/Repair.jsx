...
const Repair = () => {
  ...
  function getData() {
    axios
      .get(`${process.env.REACT_APP_API_URL}/qrancae/getmaint`)
      .then((res) => {
        setMaints(res.data);
      })
      .catch((err) => {
  ...
  function getUsers() {
    axios
      .get(`${process.env.REACT_APP_API_URL}/maint/getusers`)
      .then((res) => {
        setUsers(res.data);
      })
      .catch((err) => {
  ...
  function initializeDataTable() {
      ...
      columns: [
        ...
        { title: '케이블', data: 'cable.cable_idx' },
        {
          ...
          render: function (data) {
            ...
            if (data.maint_qr === '불량') {
              errorMessages.push(
                `QR 상태: <span style="color:red">${data.maint_qr}</span>`
              );
            }
            if (data.maint_cable === '불량') {
              errorMessages.push(
                `케이블 상태: <span style="color:red">${data.maint_cable}</span>`
              );
            }
            if (data.maint_power === '불량') {
              errorMessages.push(
                `전원 공급 상태: <span style="color:red">${data.maint_power}</span>`
              );
            }
            if (data.maint_msg) {
    ...
    const repairingTable = $('#repairing-table').DataTable({
      ...
      columns: [
        ...
        },
        {
          title: '요청 날짜',
          data: 'maint_date',
          render: (data) => formatDate(data) || '없음',
        },
        {
          title: '처리 작업자',
          data: null,
          render: function (data) {
            return data.maintUser
              ? `${data.maintUser.user_name} (${data.maintUser.user_id})`
              : '요청중';
          },
        },
        {
    ...
    $('#tab-in-progress').on('click', function () {
      const filteredData = filterFaultyMaints(maints).filter(
        (item) => item.maintUser !== null && item.maint_update === null
      );
    ...
    $('#tab-all').on('click', function () {
      repairingTable
        .clear()
        .rows.add(
          filterFaultyMaints(maints).filter((item) => item.maintUser !== null)
        )
        .draw();
      setSelectedBtn('전체');
    });
  ...
  const maintUserSelectClick = () => {
    const selectedIdxs = $('#repair-table .select-checkbox:checked')
      .map(function () {
        return $(this).data('id');
      })
      .get();
    if (selectedIdxs.length > 0) {
      ...
      // 선택된 유지보수 내역 정보 가져오기
      const selectedMaintsInfo = maints.filter((item) =>
        selectedIdxs.includes(item.maint_idx)
      );
  ...
  const handleConfirmSubmit = () => {
    axios
      .post(`${process.env.REACT_APP_API_URL}/maint/updateuser`, {
      ...
      .then(() => {
        ...
        const updatedData = maints.map((item) => {
          if (selectedMaintIdxs.includes(item.maint_idx)) {
            return {
              ...item,
              maintUser: { user_id: selectedUser },
              user_note: alarmMsg,
            };
          }
          return item;
        ...
        const updatedRepairingData = filterFaultyMaints(updatedData).filter(
          (item) => item.maintUser !== null
        );
        ...
        if (tableInstance) {
          // 비동기적으로 테이블을 새로고침
          setTimeout(() => {
            tableInstance.repairingTable
              .clear()
              .rows.add(updatedRepairingData)
              .draw();
  ...
  return (
    <div className="wrapper">
                      ...
                      <span className="tooltip-container">
                        <span className="tooltip-icon">
                          <i
                            ...
                            style={{ color: '#4574C4', cursor: 'pointer' }}
                          ></i>
                        </span>
                        <span className="tooltip-text">
                          점검 대상 케이블 목록의 행 선택하기
                        </span>
                      </span>
                    </div>
                    {rackLocationInfo && (
                ...
                <div className="card card-round">
                  <div className="card-header">
                    ...
                    <div>
                      <label
                        id="tab-all"
                        className={`btn btn-label-primary btn-sm ${selectedBtn === '전체' ? 'select-btn-label' : ''}`}
                      >
                        전체
                      </label>
                      <label
                        id="tab-in-progress"
                        className={`btn btn-label-primary btn-sm ${selectedBtn === '진행중' ? 'select-btn-label' : ''}`}
                      >
                        진행중
                      </label>
                      <label
                        id="tab-completed"
                        className={`btn btn-label-primary btn-sm ${selectedBtn === '완료' ? 'select-btn-label' : ''}`}
                      >
                        완료
                      </label>
                    </div>
      ...
      <Modal show={modalIsOpen} onHide={() => setModalIsOpen(false)} centered>
        ...
        >
                  ...
                  <ul>
                    {selectedMaintsInfo.map((item) => (
                      <li key={item.maint_idx} style={{ textAlign: 'left' }}>
                        <strong>케이블 번호 : </strong> {item.cable.cable_idx}
                        <br />
                        <strong>오류 내용</strong>
                        <br />
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" onClick={handleUserConfirm}>
              확인
            </Button>
          </Modal.Footer>
        </Modal.Dialog>
      ...
      <Modal
        ...
        onHide={() => setConfirmModalIsOpen(false)}
        centered
      >
        <Modal.Dialog className="modal-dialog" style={{ width: '700px' }}>
          ...
          <Modal.Body>
            ...
            <h5>
              작업자:{' '}
              {users.find((user) => user.user_id === selectedUser)?.user_name} (
              {selectedUser})
            </h5>
            <p>
              <strong>추가 요청사항:</strong> {alarmMsg || '없음'}
            </p>
            <h5>선택된 유지보수 내역</h5>
            <ul>
              {selectedMaintsInfo.map((item) => (
                <li key={item.maint_idx} style={{ textAlign: 'left' }}>
                  ...
                  <div style={{ marginLeft: '10px' }}>
                    {item.maint_qr === '불량' && (
                      <div>
                        - QR 상태:{' '}
                        <span style={{ color: 'red' }}>{item.maint_qr}</span>
                      </div>
                    )}
                    {item.maint_cable === '불량' && (
                      <div>
                        - 케이블 상태:{' '}
                        <span style={{ color: 'red' }}>{item.maint_cable}</span>
                      </div>
                    )}
                    {item.maint_power === '불량' && (
                      <div>
                        - 전원 공급 상태:{' '}
                        <span style={{ color: 'red' }}>{item.maint_power}</span>
                      </div>
                    )}
                  </div>
                </li>
          ...
          <Modal.Footer>
            <Button
              variant="secondary"
              style={{ backgroundColor: '#4574C4', borderColor: '#4574C4' }}
