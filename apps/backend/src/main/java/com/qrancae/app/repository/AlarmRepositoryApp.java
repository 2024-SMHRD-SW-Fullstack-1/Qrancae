...
public interface AlarmRepositoryApp extends JpaRepository<AlarmData, Long> {
    ...
    @Query("SELECT a FROM AlarmData a WHERE a.userId = :userId AND EXISTS (SELECT 1 FROM MaintApp m WHERE m.maintIdx = a.maintIdx AND m.maintUserId = :userId AND m.maintStatus = '진행중')")
